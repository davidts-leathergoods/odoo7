openerp.mail = function (session) {
     ...
     */
        ...
        init: function (parent, datasets, options) {
            ...
            this.thread = false;
            this.show_attach_message = !this.model;
        },
    ...
    mail.ThreadMessage = mail.MessageCommon.extend({
        ...
        bind_events: function () {
            ...
            this.$('.oe_attach').on('click', this.on_compose_fullmail_attach);
        ...
        */
        ...
        on_compose_fullmail_attach: function () {
            var self = this;
            if(!this.do_check_attachment_upload()) {
                return false;
            }
            ...
            $.when(recipient_done).done(function (partner_ids) {
                var context = {
                    'default_id': self.id || false,
                    'default_subject': self.subject || false,
                    'default_body': self.body || false,
                    'default_author': self.author_id || false,
                    'default_attachment_ids': self.attachment_ids || false,
                    'default_partner_ids': partner_ids || false,
                    'mail_post_autofollow': true,
                    'mail_post_autofollow_partner_ids': partner_ids || false,
                };
                if (self.is_log) {
                    _.extend(context, {'mail_compose_log': true});
                }
