...
#                'track_incoming': product.product_tmpl_id.variant_track_incoming,
    ...
    def compute_dimension_extra_price(self, cr, uid, ids, result, product_price_extra=False, dim_price_margin=False, dim_price_extra=False, context=None):
        if context is None:
            context = {}
        for product in self.browse(cr, uid, ids, context=context):
            if product.price_extra > 0.0 or 'uom' in context:
    ...
    _columns = {
        ...
        'lst_price' : fields.function(_product_lst_price, method=True, type='float', string='Sale Price', digits_compute=dp.get_precision('Sale Price')),
